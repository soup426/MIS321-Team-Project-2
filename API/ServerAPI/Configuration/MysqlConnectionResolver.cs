...
public static class MysqlConnectionResolver
{
    ...
    private static string FromMysqlUrl(string url, IConfiguration configuration)
    {
        ...
        // For MySQL 8 default auth (caching_sha2_password) on non-SSL connections (common in local dev),
        // AllowPublicKeyRetrieval enables the client to fetch the server RSA key for password exchange.
        // When SSL is required, this setting is effectively irrelevant but harmless.
        return $"Server={host};Port={port};Database={database};User Id={user};Password={password};SslMode={sslMode};AllowPublicKeyRetrieval=True;";
    }
}
