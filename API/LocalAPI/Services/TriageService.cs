...
public class TriageService : ITriageService
{
    ...
    private async Task<TriageResult> TriageWithN8nAsync(MaintenanceRequest ticket, string webhookUrl, CancellationToken ct)
    {
        ...
        var publicBaseUrl = _configuration["Uploads:PublicBaseUrl"]?.Trim();
        ...
        if (!string.IsNullOrWhiteSpace(publicBaseUrl))
        {
            var keys = await _db.MaintenanceRequestImages.AsNoTracking()
                .Where(i => i.MaintenanceRequestId == ticket.Id)
                .OrderByDescending(i => i.CreatedAt)
                .Select(i => i.StorageKey)
                .Take(3)
                .ToListAsync(ct);
            imageUrls = keys.Select(k => $"{publicBaseUrl}/uploads/{k.TrimStart('/')}").ToArray();
        }
